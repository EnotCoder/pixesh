...
// Bitmap font 5x7 для ASCII 32-126 (space through ~)
// Каждый символ = 5 байт, каждый байт = колонка (LSB = верхний ряд)
        ...
        for cx in 0..5 {
            ...
            for cy in 0..7 {
                if col & (1 << cy) != 0 {
                    for sx in 0..scale {
                        for sy in 0..scale {
                            let px = pen_x + cx as i32 * scale + sx;
                            let py = y + cy as i32 * scale + sy;
